<template>
  <v-row justify="center" class="d-block edit-form" no-gutters>
    <v-dialog
        persistent
        max-width="600px"
        v-model="dialog"
    >
      <template #activator="bindings">
        <slot name="activator" v-bind="bindings"/>
      </template>
      <v-card>
        <v-card-title>
          <span class="text-h5">Создание задачи</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row>
              <v-col
                  cols="12"
              >
                <v-text-field
                    label="Название"
                    v-model="taskName"
                    required
                ></v-text-field>
              </v-col>
              <v-col
                  cols="12"
              >
                <v-textarea
                    label="Описание"
                    v-model="taskDescription"
                    required
                ></v-textarea>
              </v-col>
              <v-col
                  cols="12"
                  sm="6"
                  md="4"
              >
                <v-menu
                    v-model="dateField"
                    :close-on-content-click="false"
                    :nudge-right="40"
                    transition="scale-transition"
                    offset-y
                    min-width="auto"
                >
                  <template v-slot:activator="{ on, attrs }">
                    <v-text-field
                        v-model="date"
                        label="Срок выполнения"
                        prepend-icon="mdi-calendar"
                        readonly
                        v-bind="attrs"
                        v-on="on"
                    ></v-text-field>
                  </template>
                  <v-date-picker
                      v-model="date"
                      @input="dateField = false"
                  ></v-date-picker>
                </v-menu>
              </v-col>
              <v-col
                  cols="12"
              >
                <span class="subheading">Приоритет</span>
                <v-chip-group
                    v-model="selection"
                    active-class="deep-purple--text text--accent-4"
                    mandatory
                >
                  <v-chip
                      @click="taskPriority = true">Высокий
                  </v-chip>
                  <v-chip
                      @click="taskPriority = false">Низкий
                  </v-chip>
                </v-chip-group>
              </v-col>
            </v-row>
          </v-container>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn
              color="blue darken-1"
              text
              @click="closeModal"
          >
            Close
          </v-btn>
          <v-btn
              color="blue darken-1"
              text
              @click="saveData"
          >
            Save
          </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-row>
</template>

<script lang="ts">
import { Component, Prop, Vue } from 'vue-property-decorator';
import { CardData } from '@/types/cardData';

@Component({
  name: 'NewCard',
  components: {},
})

export default class NewCard extends Vue {
  @Prop({ type: Array, required: true }) public taskCards!: CardData[];

  public dialog: boolean = false;
  public dateField: boolean = false;
  public taskName: string = '';
  public taskDescription: string = '';
  public taskPriority: boolean = true;
  public selection: number = 0;
  public currentDate: string = (new Date(Date.now() - (new Date()).getTimezoneOffset() * 60000)).toISOString().substr(0, 10);
  public date: string = this.currentDate;

  public createCard(title: string, value: string, date: string, isMain: boolean): CardData {
    return {
      title: title,
      value: value,
      date: date,
      is_main: isMain,
    };
  }

  public resetForm(): void {
    this.taskName = '';
    this.taskDescription = '';
    this.date = this.currentDate;
    this.selection = 0;
  }

  public closeModal(): any {
    this.dialog = false;
    this.resetForm();
  }

  public saveData(): any {
    this.dialog = false;
    const newCard: CardData = this.createCard(this.taskName, this.taskDescription, this.date, this.taskPriority);
    this.taskCards.push(newCard);
    this.resetForm();
  }
}

</script>

<style lang="scss" scoped>
.edit-form {
  flex: unset;
}
</style>
